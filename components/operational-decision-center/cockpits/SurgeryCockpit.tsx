...
const SurgeryCockpit: React.FC<SurgeryCockpitProps> = ({
  onBack,
  selectedCampus,
  setSelectedCampus,
  ...
  const surgeryTypeData = [
    { name: '择期手术', value: 120, color: '#3b82f6' },
    { name: '急诊手术', value: 45, color: '#f59e0b' },
    { name: '日间手术', value: 35, color: '#10b981' },
  ];

  const surgeryLevelData = [
    { name: '一级', value: 30, color: '#94a3b8' },
    { name: '二级', value: 50, color: '#64748b' },
    { name: '三级', value: 80, color: '#3b82f6' },
  ...
  return (
    <div className="w-full bg-[#0f172a] flex-1 min-h-0 text-white p-6 pb-24 space-y-6 overflow-y-auto">
      ...
      <div className="flex items-center justify-between mb-2 relative">
        ...
        <div className="flex items-center gap-4 z-10">
          <button 
            ...
            className="px-4 py-2 bg-blue-600/20 text-blue-400 border border-blue-600/30 rounded-lg text-sm font-medium hover:bg-blue-600/30 transition-all"
          >
            手术分析
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex flex-col gap-2">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex flex-col gap-2">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-bold">85.4</span>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2 text-xs">
            <span className="text-emerald-500">↑ 2人</span>
            <span className="text-slate-500">较昨日</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 flex flex-col h-[360px]">
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={surgeryLevelData}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#1e293b" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 12 }} />
                ...
                <Bar dataKey="value" radius={[4, 4, 0, 0]} barSize={30}>
                  {surgeryLevelData.map((entry, index) => (
                    <Cell key={`cell-${index}`} fill={entry.color} />
                  ))}
                </Bar>
              </BarChart>
